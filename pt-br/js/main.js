...
function addReceita () {
	...
	if(nome != ""){
		...
		toast("Favor digitar o nome da receita!");
	}
...
function editReceita () {
	...
	if(nome != ""){
		banco.transaction(function (tx){
			...
			toast("Receita Editada com Sucesso");	
			bb.popScreen();
		});
	}else{
		toast("Favor digitar o nome da receita!");
...
function deleteReceita (item) {
	try {
	    blackberry.ui.dialog.standardAskAsync("Deseja excluir realmente?", blackberry.ui.dialog.D_OK_CANCEL, confirmExclusao, {title : "Deletar Receita", size: blackberry.ui.dialog.SIZE_MEDIUM, position : blackberry.ui.dialog.BOTTOM});
 	}catch (e) {
    	alert("Ops: " + e);
  	}
				...
				tx.executeSql('SELECT * FROM receitas WHERE codigo="' + codigo + '"', [], function(tx, results) {
			      	...
			      	for(i=0; i < quant; i++) {
			      	  	...
			      	  	if(receita.tempoDePreparo != ""){
			      	  		tempoDePreparo = "\nTempo de Preparo: " + receita.tempoDePreparo + "min";
			      	  	}
			      	  	if(receita.rendimento != ""){
			      	  		rendimento = encode("\nRendimento: " + receita.rendimento + " porções");
			      	  	}
						...
						var conteudo = "Confira a minha nova receita!" + "\n\nNome da Receita: " + encode(receita.nome) + tempoDePreparo + rendimento + ingredientes + modoDePreparo + infoAdicionais + "\n\nPor CookBook para BlackBerry10";
			     	}
